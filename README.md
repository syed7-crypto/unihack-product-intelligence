...
The catalogue workflow includes typed CSV input, controlled manufacturer/brand/reference resolution, explicitly approved manufacturer URLs, exact MPN verification, web/PDF normalization, generic single-row enrichment, safe row-isolated batch orchestration, the exact 252-column delivery schema, controlled attribute mapping, evaluation-only comparison, and the unified review/exception layer.
